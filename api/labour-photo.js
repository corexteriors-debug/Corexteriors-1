...
module.exports = async function handler(req, res) {
    ...
    try {
        const { sessionToken, calendarEventId, jobTitle, photoData, tag } = req.body || {};
        // photoData: base64 data URL, compressed client-side to ~1MB
        const normalizedTag = (tag === 'before' || tag === 'after') ? tag : null;
        ...
        // Append photo to the job's entry in the daily log
        ...
        const photoEntry = { url: blob.url, tag: normalizedTag, at: new Date().toISOString() };
        ...
        if (job) {
            ...
            log.jobs.push({ calendarEventId, jobTitle: jobTitle || 'Job', clockIn: new Date().toISOString(), clockOut: null, photos: [photoEntry], workerNotes: [] });
        }
