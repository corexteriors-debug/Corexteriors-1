...
async function generateContractPDF(est, signatureData) {
    ...
    const serviceAddress = est.address       || est.clientAddress || '';
    ...
    const dateOfService = est.saleDate
        ? new Date(est.saleDate).toLocaleDateString('en-CA', { year: 'numeric', month: 'long', day: 'numeric' })
    ...
    const qtyX  = R - 166;      // quantity column
    const priceX = R - 48;      // price column (right-aligned)
    const rowH  = 22;

    // Table header
    page.drawRectangle({ x: L, y: y - rowH + 6, width: R - L, height: rowH, color: rgb(0.93, 0.93, 0.93) });
    drawTableBorder(page, L, y - rowH + 6, R - L, rowH, lightGray);
    ...
    const completionDate = est.saleDate
        ? new Date(new Date(est.saleDate).getTime() + 86400000)
    ...
    termsList.forEach(term => {
        ...
        const wrapped = wrapText(full, maxChars);
        wrapped.forEach((line, idx) => {
            if (idx === 0) {
                // Draw bold label, then regular text on same line
    ...
    page.drawText('Client SignatureDate: ______________', { x: L,            y: y - 59, size: 8, font, color: gray });
    page.drawText('Contractor SignatureDate: ______________', { x: R - sigHalf, y: y - 59, size: 8, font, color: gray });
...
function drawCheckbox(page, x, y, checked) {
    const size = 8;
    page.drawRectangle({ x, y: y - size, width: size, height: size, borderColor: rgb(0, 0, 0), borderWidth: 0.8, color: rgb(1, 1, 1) });
    if (checked) {
...
function drawTableBorder(page, x, y, w, h, color) {
    page.drawRectangle({ x, y, width: w, height: h, borderColor: color, borderWidth: 0.5, color: rgb(0, 0, 0, 0) });
}
