...
// Non-routable helper (Vercel ignores files prefixed with "_") dispatched from
// api/remind.js?task=sync-fb-leads to stay under the Hobby-plan 12-function cap.
//
// Pulls new rows from the Facebook Lead Ads sheet (Sheet2 of the "Residential
// Leads - London" spreadsheet) and creates a quick-lead for each one, tagged
// source: 'Facebook Ads'.
...
async function runFbLeadsSync() {
    const spreadsheetId = (process.env.GOOGLE_SHEET_ID_FB || '').trim();
    if (!spreadsheetId) throw new Error('GOOGLE_SHEET_ID_FB not configured');
